...
const ClockSvg = () => (
  <svg width="64" height="64" viewBox="0 0 64 64" fill="none" style={{ animation: "spin 4s linear infinite" }}>
    ...
    <defs><linearGradient id="cg" x1="0" y1="0" x2="64" y2="64"><stop stopColor="#00C8FF" /><stop offset="1" stopColor="#0077FF" /></linearGradient></defs>
  </svg>
);
    ...
    <div className="min-h-screen bg-[#0D1B2A] flex items-center justify-center px-4 py-8 relative">
      ...
      <div className="w-full max-w-lg" style={{ animation: "fadeInUp 0.4s ease-out" }}>
        <div className="glass-card p-10 sm:p-12 text-center">
          ...
          <div className="p-4 rounded-xl text-sm text-left flex items-start gap-3 mb-8" style={{ background: "rgba(0,200,255,0.1)", border: "1px solid rgba(0,200,255,0.25)", fontFamily: "'Plus Jakarta Sans', sans-serif", color: "rgba(255,255,255,0.8)" }}>
            <span>📱</span>
            <div>
              ...
              <React.Fragment key={s.label}>
                <div className="flex flex-col items-center gap-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  <span className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{
                    background: s.done ? "rgba(0,229,160,0.2)" : s.active ? "rgba(0,200,255,0.2)" : "rgba(255,255,255,0.05)",
                    border: `2px solid ${s.done ? "var(--success)" : s.active ? "var(--primary)" : "rgba(255,255,255,0.15)"}`,
                ...
                {i < steps.length - 1 && (
                  <div className="flex-1 h-px mx-2 mt-[-24px]" style={{ background: s.done ? "var(--success)" : "rgba(255,255,255,0.1)" }} />
                )}
              </React.Fragment>
            ))}
          </div>
