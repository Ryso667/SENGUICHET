    ...
    <div className="min-h-screen bg-[#0D1B2A] flex">
      <aside className="hidden lg:flex flex-col w-[260px] fixed h-full z-20" style={{ background: "#060710", borderRight: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="p-6 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <p className="text-xl font-bold" style={{ fontFamily: "Outfit, sans-serif", fontWeight: 800, background: "linear-gradient(135deg, var(--accent), var(--error))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>SenGuichet</p>
        </div>
        <div className="p-4 border-b flex items-center gap-3" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0077FF] to-[#FF4D6D] flex items-center justify-center text-white font-bold text-sm">
          ...
          <div>
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px]" style={{ background: "rgba(255,77,109,0.2)", color: "var(--accent)", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Admin</span>
          </div>
        </div>
          ...
          {adminNavItems.map((item) => (
            <button
              ...
              style={{
                ...
                background: activeNav === item.key ? "rgba(0,119,255,0.1)" : "transparent",
                color: activeNav === item.key ? "var(--accent)" : "rgba(255,255,255,0.5)",
        ...
        <div className="p-3">
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm" style={{ background: "rgba(255,77,109,0.1)", border: "1px solid rgba(255,77,109,0.2)", color: "var(--error)", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Déconnexion
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 lg:ml-[260px] flex flex-col">
        <header className="sticky top-0 z-10 px-4 sm:px-8 py-4" style={{ background: "rgba(10,11,26,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleLogout} className="lg:hidden px-3 py-2 rounded-xl text-xs" style={{ background: "rgba(255,77,109,0.1)", border: "1px solid rgba(255,77,109,0.2)", color: "var(--error)", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Déconnexion
            </button>
          </div>
        </header>
          ...
          <div className="flex justify-around">
            {adminNavItems.map((item) => (
              <button
                ...
                style={{
                  color: activeNav === item.key ? "var(--accent)" : "rgba(255,255,255,0.4)",
                  background: activeNav === item.key ? "rgba(0,119,255,0.1)" : "transparent",
                  fontFamily: "'Plus Jakarta Sans', sans-serif",
                }}
              >
